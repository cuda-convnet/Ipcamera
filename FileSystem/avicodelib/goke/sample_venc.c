...
#if  (defined (AI_TYPE_SUNRISE)||defined (AI_TYPE_ALI_DMY))
	
	#include "face_module/face_module.h"
...
unsigned long long G_curIndex = 1;
...
HI_S32 SAMPLE_VENC_1080P_MJPEG_JPEG2(HI_VOID)

{
    ...
    ******************************************/
    ...
    /******************************************
     step 2: mpp system init.
    ******************************************/
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_SYS_Init(&stVbConf);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("system init failed with %d!\n", s32Ret);
    ...
    ******************************************/
    ...
    stViConfig.enNorm     = VIDEO_ENCODING_MODE_AUTO;
    stViConfig.enViChnSet = VI_CHN_SET_NORMAL;
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VI_StartVi(&stViConfig);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("start vi failed!\n");
    ...
    ******************************************/
    ...
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VI_BindVpss(stViConfig.enViMode);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Vi bind Vpss failed!\n");
    ...
    stVpssChnAttr.s32SrcFrameRate = -1;
    stVpssChnAttr.s32DstFrameRate = -1;
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VPSS_EnableChn(VpssGrp, VpssChn, &stVpssChnAttr, &stVpssChnMode, HI_NULL);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Enable vpss chn failed!\n");
        goto END_VENC_MJPEG_JPEG_4;
    ...
    ******************************************/
    ...
    VpssChn = 0;
    VencChn = 0;
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VENC_Start(VencChn, enPayLoad, \
                                    gs_enNorm, enSize[0], enRcMode, u32Profile,ROTATE_NONE);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start Venc failed!\n");
    ...
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VENC_BindVpss(VencChn, VpssGrp, VpssChn);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start Venc failed!\n");
        goto END_VENC_MJPEG_JPEG_5;
...
HI_S32 SAMPLE_VENC_1080P_MJPEG_JPEG(HI_VOID)
{
    ...
    ******************************************/
    ...
    stVpssGrpAttr.bDciEn = HI_FALSE;
    //stVpssGrpAttr.bSharpenEn = HI_FALSE;
    s32Ret = HI_SUCCESS;//SAMPLE_COMM_VPSS_StartGroup(VpssGrp, &stVpssGrpAttr);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start Vpss failed!\n");
        goto END_VENC_MJPEG_JPEG_2;
...
void SaveYuvData(VIDEO_FRAME_INFO_S *pFrame,VIDEO_FRAME_S* pVBuf,int channel,unsigned long long time,vpss_video * my_vpss_video)
{
   ...
   // memcpy(myTmpBuff+dataoffset, pVBufVirt_C, pVBuf->u32Width* pVBuf->u32Height/2);
	...
	video_data_callback(pFrame,pVBufVirt_Y, pVBuf->u32Width,pVBuf->u32Height, channel,enPixelFormat,time);
#endif
   //printf("video_data_callback %d!\n", channel);
...
HI_VOID* GetYuvDataFormVPSS(VPSS_GRP Grp, VPSS_CHN Chn, HI_U32 u32FrameCnt,vpss_video * my_vpss_video)
{
    ...
    while (u32Cnt--)
    {
		...
		video_data_callback(&my_vpss_video->stFrame,G_curIndex%0xffffffff, Chn);
       // if(my_vpss_video->VpssChn == 0)
		//	s32Ret = HI_MPI_VO_SendFrame(0,0,&my_vpss_video->stFrame,s32MilliSec);

		/*
        bSendToVgs = 1;//((my_vpss_video->stFrame.stVFrame.enCompressMode > 0) || (my_vpss_video->stFrame.stVFrame.enVideoFormat > 0));
        ...
        if (bSendToVgs)
        {
            ...
            SaveYuvData(&my_vpss_video->stFrame,&stFrmInfo.stVFrame,Chn,my_vpss_video->stFrame.stVFrame.u64pts,my_vpss_video);
...
FrameList  myFrameList[MAX_FRAME_NUMBER];

int myFrameListFront = 0;
int myFrameListBack = 0;
int myFrameListCount = 0;
...
RectList  myRectList[MAX_FRAME_NUMBER];

int myRectListFront = 0;
int myRectListBack = 0;
int myRectListCount = 0;
...
#define MAX_FACE_VIDEO_EAGE_W 4

#define MAX_FRAME_SIZE_VENC (1920*1080*3/2)



int showOneFaceRect2(rect_t *pRect,HI_U8 *facebtBuffer)

{
	int rectCounty = 0;
			unsigned char * facebtBuffer2 = facebtBuffer + pRect->top*MAX_FACE_VIDEO_WIDTH+pRect->left;
			unsigned short * facebtBuffer3 = facebtBuffer +1920*1080;
			int myw = pRect->right - pRect->left;
			int myw_lap = pRect->right - pRect->left - MAX_FACE_VIDEO_EAGE_W*2 ;
	...
	for(rectCounty = 0;rectCounty < pRect->bottom - pRect->top ;rectCounty ++)
		{
			...
			for(rectCountx = 0;rectCountx < myw ;rectCountx ++)
				{

				
					
					if(pRect->bottom - pRect->top - rectCounty < MAX_FACE_VIDEO_EAGE_W)
					{
						facebtBuffer2[rectCountx] = 0x96;//0x7c00;
						continue;
					}
					else if(rectCountx == MAX_FACE_VIDEO_EAGE_W)
					{
						facebtBuffer2[rectCountx] = 0x96;//0x7c00;
						rectCountx+= myw_lap;
						continue;
					 ...
					 if(pRect->right - pRect->left - rectCountx < MAX_FACE_VIDEO_EAGE_W)
					{
						facebtBuffer2[rectCountx] = 0x96;//0x7c00;
						continue;
					}
					...
					if(rectCounty < MAX_FACE_VIDEO_EAGE_W)
					{
						facebtBuffer2[rectCountx] = 0x96;//0x7c00;
						continue;
					}
					
					if(rectCountx < MAX_FACE_VIDEO_EAGE_W)
					{
						facebtBuffer2[rectCountx] = 0x96;//0x7c00;
					} 
	...
	for(rectCounty = 0;rectCounty < (pRect->bottom - pRect->top) /2;rectCounty ++)
		{
			int rectCountx = 0;
			unsigned short * facebtBuffer4 = facebtBuffer3 + (pRect->top/2+rectCounty)*MAX_FACE_VIDEO_WIDTH/2+pRect->left/2;
			for(rectCountx = 0;rectCountx < (pRect->right - pRect->left)/2 ;rectCountx ++)
				{
					if(rectCounty < MAX_FACE_VIDEO_EAGE_W/2)
					{
						facebtBuffer4[rectCountx] = 0x6244;//0x7c00;
						continue;
					}
					
					if(rectCountx < MAX_FACE_VIDEO_EAGE_W/2)
					{
						facebtBuffer4[rectCountx] = 0x6244;//0x7c00;
						continue;
					}
					
					if((pRect->bottom - pRect->top)/2 - rectCounty < MAX_FACE_VIDEO_EAGE_W/2)
					{
						facebtBuffer4[rectCountx] = 0x6244;//0x7c00;
						continue;
					}
					else if(rectCountx == MAX_FACE_VIDEO_EAGE_W/2)
					{
						facebtBuffer4[rectCountx] = 0x6244;//0x7c00;
						rectCountx+= myw_lap/2;
						continue;
					}
					
					if((pRect->right - pRect->left)/2 - rectCountx < MAX_FACE_VIDEO_EAGE_W/2)
					{
						facebtBuffer4[rectCountx] = 0x6244;//0x7c00;
						continue;
					}
...
int showRectOnVenc2(rect_t *pRect,int count,HI_U8 *buffer)
{
	int rectCount = 0;
	for(rectCount = 0;rectCount < count ;rectCount ++)
	{
		if((pRect[rectCount].right <=MAX_FACE_VIDEO_WIDTH )&&(pRect[rectCount].bottom <=MAX_FACE_VIDEO_HEIGHT )&&(pRect[rectCount].top >= 0 )&&(pRect[rectCount].left  >= 0 ))
			showOneFaceRect2(&pRect[rectCount],buffer);
		else
			printf("error rect\n");
...
pthread_mutex_t Sunrise_Face_mutex;


void  Do_Hisi_Ali_DMY_Face_Info(unsigned long long index,tracker_t *pRect,int count)
{

	printf("Do_Hisi_Sunrise_Face_Info %lld\n",index);
	...
	if(count == 0)
        ...
        pthread_mutex_lock(&Sunrise_Face_mutex);
	myRectList[myRectListFront].count = count;
	myRectList[myRectListFront].index = index;
	memcpy(&myRectList[myRectListFront],pRect,sizeof(tracker_t)*count);
	
		myRectListFront++;
		myRectListCount++;
		myRectListFront%=MAX_FRAME_NUMBER;

        pthread_mutex_unlock(&Sunrise_Face_mutex);

		
}

void  Do_Hisi_Sunrise_Face_Info(unsigned long long index,tracker_t *pRect,int count)
{
...
void  Do_Hisi_Sunrise_Face_Info2(unsigned long long index,tracker_t *pRect,int count)
{

	//printf("Do_Hisi_Sunrise_Face_Info %lld\n",index);

	int frameIndex = 0;
	int tmp_frameIndex = 0;

    HI_S32 s32MilliSec = -1;


	for( frameIndex = 0; frameIndex < MAX_FRAME_NUMBER;frameIndex++)
	{
		if(myFrameList[(myFrameListBack+frameIndex)%MAX_FRAME_NUMBER].index == 0)
			continue;
		else if(myFrameList[(myFrameListBack+frameIndex)%MAX_FRAME_NUMBER].index == index)
			{

/*
			if(myFrameList[myFrameListBack].index == index)
			{
				HI_U8 *vaddr = HI_MPI_SYS_Mmap(myFrameList[myFrameListBack].stFrmInfo.stVFrame.u32PhyAddr[0], MAX_FRAME_SIZE_VENC);
				

				showRectOnVenc2(pRect, count,vaddr);
				
				if (vaddr)
					HI_MPI_SYS_Munmap(vaddr,MAX_FRAME_SIZE_VENC);
			}*/
			//printf("index %lld %lld\n",myFrameList[(myFrameListBack+frameIndex)%MAX_FRAME_NUMBER].index, index);
	//showRectOnVenc2(pRect, count,myFrameList[(myFrameListBack+frameIndex)%MAX_FRAME_NUMBER].pVirAddr);

	int rectCount = count;
		for(rectCount = 0;rectCount < count ;rectCount ++)
		{
			if((pRect[rectCount].rect.right <=MAX_FACE_VIDEO_WIDTH )&&(pRect[rectCount].rect.bottom <=MAX_FACE_VIDEO_HEIGHT )&&(pRect[rectCount].rect.top >= 0 )&&(pRect[rectCount].rect.left  >= 0 ))
				showOneFaceRect2(&(pRect[rectCount].rect),myFrameList[(myFrameListBack+frameIndex)%MAX_FRAME_NUMBER].pVirAddr);
			else
				printf("error rect\n");
	...
	return;
			/*
							int s32Ret = HI_MPI_VENC_SendFrame(0, &(myFrameList[myFrameListBack].stFrmInfo),	s32MilliSec);
					
							//printf("VENC3 send frame %x %lld\n",s32Ret,myFrameList[myFrameListBack].index);
							myFrameList[myFrameListBack].index = 0;
							HI_MPI_VB_ReleaseBlock(myFrameList[myFrameListBack].hBlock);
							
							if(HI_NULL != myFrameList[myFrameListBack].pVirAddr)
							{
								HI_MPI_SYS_Munmap((HI_VOID*)myFrameList[myFrameListBack].pVirAddr,MAX_FRAME_SIZE_VENC );
								//my_vpss_video->stMem.u32PhyAddr = HI_NULL;
							}
							
							//s32Ret = HI_MPI_VPSS_ReleaseChnFrame(0, 0, &(myFrameList[myFrameListBack].stFrmInfo));
							myFrameListBack++;
							myFrameListCount--;
							myFrameListBack%=MAX_FRAME_NUMBER;*/
			}
...
static int FillTimeStamp2(void* addr, unsigned long long* timestamp)
{
    char* addrp = (char*)addr;
    int i = 0;
    for (i = 15; i >= 0; i--)
        addrp[15 - i] = 0x50 | ((unsigned char)(((*timestamp) & ((unsigned long long)0xf << (i * 4))) >> (i * 4)));
	//printf("FillTimeStamp = %lld\n",*timestamp);
...
void copyFrameToBuffer(VB_POOL hPool ,VIDEO_FRAME_INFO_S *frameData)
{



	
	//VIDEO_FRAME_INFO_S *frameData = &(myFrameList[myFrameListFront].stFrameList);

	
	FrameList *FrameListData = &(myFrameList[myFrameListFront]);
	
	myFrameListFront++;
	myFrameListCount++;
	myFrameListFront%=MAX_FRAME_NUMBER;


	//VIDEO_FRAME_INFO_S stFrmInfo;

    //VB_BLK  hBlock; 
    FrameListData->index = G_curIndex;

	while ((FrameListData->hBlock = HI_MPI_VB_GetBlock(hPool,MAX_FRAME_SIZE_VENC, HI_NULL)) == VB_INVALID_HANDLE)
	{
		;
	}
	void * u32PhyAddr = HI_MPI_VB_Handle2PhysAddr(FrameListData->hBlock);

	 FrameListData->pVirAddr = (HI_U8*) HI_MPI_SYS_Mmap(u32PhyAddr,MAX_FRAME_SIZE_VENC );


	if (FrameListData->pVirAddr == HI_NULL)
	{
		//printf("Mem dev may not open\n");
		//VPSS_Restore(Grp, Chn,my_vpss_video);
		return ;
	...
	memset(&(FrameListData->stFrmInfo.stVFrame), 0, sizeof(VIDEO_FRAME_S));
	...
	u32PicLStride = frameData->stVFrame.u32Stride[0];
	u32PicCStride = frameData->stVFrame.u32Stride[0];
	u32LumaSize = frameData->stVFrame.u32Stride[0] *frameData->stVFrame.u32Height;
	u32Width	= frameData->stVFrame.u32Width;
	u32Height	= frameData->stVFrame.u32Height;

	
	memcpy(&(FrameListData->stFrmInfo), frameData, sizeof(VIDEO_FRAME_INFO_S));

	
	//memcpy(&stTask.stImgOut , &(FrameListData->stFrmInfo), sizeof(VIDEO_FRAME_INFO_S));



	FrameListData->stFrmInfo.stVFrame.u32PhyAddr[0] = u32PhyAddr;
	FrameListData->stFrmInfo.stVFrame.u32PhyAddr[1] = FrameListData->stFrmInfo.stVFrame.u32PhyAddr[0] + u32LumaSize;
	FrameListData->stFrmInfo.stVFrame.u32PhyAddr[2] = FrameListData->stFrmInfo.stVFrame.u32PhyAddr[1] + u32ChrmSize;

	FrameListData->stFrmInfo.stVFrame.pVirAddr[0] = FrameListData->pVirAddr;
	FrameListData->stFrmInfo.stVFrame.pVirAddr[1] = (HI_U8*) FrameListData->stFrmInfo.stVFrame.pVirAddr[0] + u32LumaSize;
	FrameListData->stFrmInfo.stVFrame.pVirAddr[2] = (HI_U8*) FrameListData->stFrmInfo.stVFrame.pVirAddr[1] + u32ChrmSize;

	FrameListData->stFrmInfo.stVFrame.u32Width 	= u32Width;
	FrameListData->stFrmInfo.stVFrame.u32Height	= u32Height;
	FrameListData->stFrmInfo.stVFrame.u32Stride[0] = u32PicLStride;
	FrameListData->stFrmInfo.stVFrame.u32Stride[1] = u32PicCStride;
	FrameListData->stFrmInfo.stVFrame.u32Stride[2] = u32PicCStride;

	FrameListData->stFrmInfo.stVFrame.enCompressMode = COMPRESS_MODE_NONE;
	FrameListData->stFrmInfo.stVFrame.enPixelFormat  = frameData->stVFrame.enPixelFormat;
	FrameListData->stFrmInfo.stVFrame.enVideoFormat  = frameData->stVFrame.enVideoFormat;

	FrameListData->stFrmInfo.stVFrame.u64pts	  = frameData->stVFrame.u64pts;
	FrameListData->stFrmInfo.stVFrame.u32TimeRef = frameData->stVFrame.u32TimeRef;

	FrameListData->stFrmInfo.u32PoolId = hPool;
	...
	int s32Ret = HI_MPI_VGS_BeginJob(&hHandle);
	if (s32Ret != HI_SUCCESS)
	{
		printf("HI_MPI_VGS_BeginJob failed\n");
		//my_vpss_video->hHandle = -1;
	...
	memcpy(&stTask.stImgIn, frameData, sizeof(VIDEO_FRAME_INFO_S));
...
HI_VOID* Hisi_GetFrameVPSSToVencThreadEx(VPSS_GRP Grp, VPSS_CHN Chn)
{
    HI_CHAR szYuvName[128];
    HI_CHAR szPixFrm[10];
    HI_U32 u32Cnt = 0;
...
//	int curIndex = 0;
	pthread_mutex_init(&Sunrise_Face_mutex, NULL) ;
	...
	for( frameIndex = 0; frameIndex < MAX_FRAME_NUMBER;frameIndex++)
	{
		myFrameList[frameIndex].index = 0;
	}
    
	VB_POOL hPool  = HI_MPI_VB_CreatePool(MAX_FRAME_SIZE_VENC, MAX_FRAME_NUMBER, HI_NULL);
    ...
    while (1)
    {
			...
			while(myRectListCount>0)
        	{
			
			//;
			//memcpy(myRectList[myFrameListBack].myRect,pRect,sizeof(rect_t)*count);
			//è¿™é‡ŒåŽ‹ç¼©jpeg
			
			
			mytest_snap++;
			
			if(mytest_snap %20 == 1&&0)
				{
				 ...
				 VENC_RECV_PIC_PARAM_S stRecvParam;
					 ...
					 ******************************************/
					 ...
					 s32Ret = HI_MPI_VENC_StartRecvPicEx(3, &stRecvParam);
					 if (HI_SUCCESS != s32Ret)
					 {
			...
			s32Ret = HI_MPI_VENC_SendFrame(3, &(myFrameList[myRectListBack].stFrmInfo), s32MilliSec);
			
			printf("venc2 jpeg frame %x \n",s32Ret);
			
			if (HI_SUCCESS == s32Ret)
							  {
			
			 s32Ret = SAMPLE_COMM_VENC_SnapProcess(3,HI_TRUE,HI_FALSE);
				   if (HI_SUCCESS != s32Ret)
				   {
					   printf("%s: sanp process failed!\n", __FUNCTION__);
					   //break;
			...
			//Do_Hisi_Sunrise_Face_Info2(myRectList[myRectListBack].index,myRectList[myRectListBack].myRect,myRectList[myRectListBack].count);
			myRectList[myRectListBack].count = 0;
			
        pthread_mutex_lock(&Sunrise_Face_mutex);
				myRectListBack++;
				myRectListCount--;
				myRectListBack%=MAX_FRAME_NUMBER;
        pthread_mutex_unlock(&Sunrise_Face_mutex);

        	}

        if (HI_MPI_VPSS_GetChnFrame(Grp, Chn, &stFrame/*(myFrameList[myFrameListFront].stFrmInfo)*/, s32MilliSec) != HI_SUCCESS)
        {
            printf("Get frame fail \n");
            usleep(30000);
            continue;
...
/*
		...
		s32Ret = HI_MPI_VENC_SendFrame(3, &stFrame,	s32MilliSec);
		...
		*/
		...
		copyFrameToBuffer( hPool ,&stFrame);
		//s32Ret = HI_MPI_VENC_SendFrame(0, &stFrame,	s32MilliSec);
				

#if  (defined (AI_TYPE_ALI_DMY) )
		G_curIndex++;
		video_data_callback(&stFrame,G_curIndex%0xffffffff, Chn);
#endif

		s32Ret = HI_MPI_VPSS_ReleaseChnFrame(Grp, Chn, &stFrame);

		if(myFrameListCount >= 20)
			{
				s32Ret = HI_MPI_VENC_SendFrame(0, &(myFrameList[myFrameListBack].stFrmInfo),	s32MilliSec);
				
				//printf("VENC2 send frame %x %lld\n",s32Ret,myFrameList[myFrameListBack].index);
				myFrameList[myFrameListBack].index = 0;
				HI_MPI_VB_ReleaseBlock(myFrameList[myFrameListBack].hBlock);
				...
				//s32Ret = HI_MPI_VPSS_ReleaseChnFrame(Grp, Chn, &(myFrameList[myFrameListBack].stFrmInfo));
				myFrameListBack++;
				myFrameListCount--;
				myFrameListBack%=MAX_FRAME_NUMBER;
		...
		/*
		VIDEO_FRAME_INFO_S *frameData = &(myFrameList[myFrameListFront].stFrmInfo);
		FrameList *FrameListData = &(myFrameList[myFrameListFront]);
...
if(0)
			...
			void *vaddr = HI_MPI_SYS_Mmap(frameData->stVFrame.u32PhyAddr[0], 2 * sizeof(uint64_t));
				   if (vaddr == NULL)
				   {
					   printf("Mmap fail!\n");
				   }
				   ...
				   if (vaddr)
				   {
					   //curIndex = get_system_time_ms();
					   FrameListData->index = curIndex;
					   FillTimeStamp2(vaddr, &curIndex);
					   
				   }
			
				   if (vaddr)
					   HI_MPI_SYS_Munmap(vaddr, 2 * sizeof(unsigned long long));
			
				   printf("send2 frame to vo\n");
				   int ret = HI_MPI_VO_SendFrame(0, 0, frameData, 2 * 1000); // always send bt
				   if (ret != 0)
				   {
					  // SMART_ERR("send vo frame!ret=%#x\n", vpssDev, vpssChn, ret);
					  // HI_MPI_VPSS_ReleaseChnFrame(vpssDev, vpssChn, &frameData);
					  // sleep(1);
					  // continue;
				   }
		...
		if(myFrameListCount > MAX_FRAME_NUMBER -10 )
		//if(1)
		{
		...
		//s32Ret = HI_MPI_VO_SendFrame(0,0,&my_vpss_video->stFrame,s32MilliSec);
			...
			printf("VENC send frame %lld\n",myFrameList[myFrameListBack].index);
			s32Ret = HI_MPI_VPSS_ReleaseChnFrame(Grp, Chn, &(myFrameList[myFrameListBack].stFrmInfo));
...
void * Hisi_GetFrameJPEGThread(void * ctrl)
{

	printf("SAMPLE_VENC_1080P_MJPEG_JPEG start\n");
	/*
        while (1)
    {
       int s32Ret = SAMPLE_COMM_VENC_SnapProcess(3,HI_TRUE,HI_FALSE);
        if (HI_SUCCESS != s32Ret)
        {
            printf("%s: sanp process failed!\n", __FUNCTION__);
            //break;
	...
	printf("SAMPLE_VENC_1080P_MJPEG_JPEG over\n");

}

void * Hisi_GetFrameVPSSToVencThread(void * ctrl)
{

	while(1)
		{
			Hisi_GetFrameVPSSToVencThreadEx(0,0);
		}
}

#endif
				
#if  (defined (AI_TYPE_ALI_DMY)||defined (AI_TYPE_TULING))		
int info_data_callback_ex(face_info * info,unsigned int index,void *user)
{
	...
	//printf("info_data_callback_ex ok\n");
        ...
        pthread_mutex_lock(&Sunrise_Face_mutex);
	myRectList[myRectListFront].count = info->rect_num;
	myRectList[myRectListFront].index = index;
	//memcpy(&myRectList[myRectListFront],pRect,sizeof(tracker_t)*count);
	...
	for(i = 0;i < info->rect_num; i ++ )
	{
		...
		myRectList[myRectListFront].trackers[i].rect.top = info->rects[i].top;
		myRectList[myRectListFront].trackers[i].rect.left = info->rects[i].left;
		myRectList[myRectListFront].trackers[i].rect.right = info->rects[i].right;
		myRectList[myRectListFront].trackers[i].rect.bottom = info->rects[i].bottom;
...
HI_S32 Hisi_StartVideoStream(GK_MEDIA_ST media)
{
	...
	******************************************/
	...
	/******************************************
		step 2: start VO SD0
		******************************************/
#if (defined (AI_TYPE_SUNRISE) || defined (AI_TYPE_TULING))
    ...
    ******************************************/
	...
	if(s32ChnNum >= 1)
	{
		...
		if (HI_SUCCESS != s32Ret)
		{
			SAMPLE_PRT("Enable 4 vpss chn failed!\n");
			goto END_VENC_1080P_CLASSIC_4;
		}
...
//è¿™é‡Œç¼“å­˜å¸§æ•°æ®
		...
		//åˆ›å»ºçº¿ç¨‹ä»Ž vpssé‡ŒèŽ·å–å›¾åƒï¼Œé€å¸§åˆ°venc

		pthread_t getFrame_vpss_to_venc;
		pthread_create(&getFrame_vpss_to_venc,  0, Hisi_GetFrameVPSSToVencThread, (void*)NULL);
	...
	SAMPLE_COMM_VENC_SnapStart(VencChn, &stSize,HI_FALSE);

#endif
	
	//Hisi_GetFrameJPEGThread(NULL);
	printf("\t step  10: \n");
	/******************************************
	 step 6: stream venc process -- get stream, then save it to file. 
	******************************************/
...
#if  (defined (AI_TYPE_ALI_DMY) ||defined (AI_TYPE_SUNRISE)||defined (AI_TYPE_TULING))
	...
	SetDepth(0,3,&vpss_main);
	//SetDepth(0,1);
 #if  (defined (AI_TYPE_SUNRISE))
	pthread_t gs_Getjpeg;

	pthread_create(&gs_Getjpeg, 0, Hisi_GetFrameJPEGThread, (void*)NULL);
#endif
