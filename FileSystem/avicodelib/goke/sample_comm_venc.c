...
extern ENC_DATA_GET_CALLBACK video_data_get_callback;
...
HI_S32 SAMPLE_COMM_VENC_SaveSnap(VENC_STREAM_S* pstStream, HI_BOOL bSaveJpg, HI_BOOL bSaveThm)
{
	...
	sprintf(acFile, "/nfs/snap_%d.jpg", gs_s32SnapCnt);
                    ...
                    if(1)
                    	{
							...
							int x;
							//printf("get v = %d\n",i);
							for (x=0; x< stStream.u32PackCount; x++)
							{	
								frame_data_len += stStream.pstPack[x].u32Len-stStream.pstPack[x].u32Offset;											
							}

							new_buf_ptr = (unsigned char *)malloc(frame_data_len + GK_VIDEO_DATA_OFFSET);
							if( new_buf_ptr )
							{				
								...
								pheader->index = i;
								pheader->type = 1; //#define VIDEO_FRAME 1
								pheader->iLength = frame_data_len;
								pheader->ulTimeSec = stStream.pstPack[0].u64PTS / 100000;
								pheader->ulTimeUsec = stStream.pstPack[0].u64PTS % 100000;
								
								
								for (x=0; x< stStream.u32PackCount; x++)
								{					

									data_ptr = stStream.pstPack[x].pu8Addr+stStream.pstPack[x].u32Offset;
									data_len = stStream.pstPack[x].u32Len - stStream.pstPack[x].u32Offset;
								...
								if( stStream.u32PackCount > 1 )
									pheader->iFrameType = 1;
								else
									pheader->iFrameType = 0;

								/*
								 if( video_data_get_callback != NULL )
								 {		   
								 	video_data_get_callback((char*)new_buf_ptr,frame_data_len + sizeof(BUF_FRAMEHEADER),i);
								 }
								 */

								 if( p_gVideolist[i] != NULL)
								  {						
								 // printf("HisiInsertVideoData ..%x.\n",new_buf_ptr);
					   				HisiInsertVideoData(p_gVideolist[i],(char*)new_buf_ptr,frame_data_len + GK_VIDEO_DATA_OFFSET,i);

								  	//free(new_buf_ptr);
								//  printf("HisiInsertVideoData ...over\n");
