...
void set_config_st_default(IPCAM_ALL_INFO_ST * pIpcam_st)
{
	...
	{
		pIpcam_st->rtsp_st.url_port[i] = 554;
		pIpcam_st->rtsp_st.server_port[i] = 16100;
	}
	...
	if( MAX_RTSP_NUM == 3 )
	strcpy(pIpcam_st->rtsp_st.stream_url[2],"2.ch");
	...
	for( i = 0; i < MAX_RTSP_NUM; i++)
	{
		if( i == 0 )
		{
			...
			pIpcam_st->onvif_st.vcode[i].BitrateLimit  = 999999;// 999999 is stand for use default value from ipcam_enc_config.txt	
			...
			pIpcam_st->onvif_st.vcode[i].GovLength = 1;
		...
		{
			strcpy(pIpcam_st->onvif_st.profile_st[i].profilename,"Profile_2");
			strcpy(pIpcam_st->onvif_st.profile_st[i].profiletoken,"Profile_Token_2");
			strcpy(pIpcam_st->onvif_st.input[i].profilename,"Profile_input_2");
			strcpy(pIpcam_st->onvif_st.input[i].profiletoken,"Profile_inputToken_2");			
			pIpcam_st->onvif_st.input[i].w = 320;
			pIpcam_st->onvif_st.input[i].h = 240;

			strcpy(pIpcam_st->onvif_st.vcode[i].Name,"Profile_vcode_2");
			strcpy(pIpcam_st->onvif_st.vcode[i].token,"Profile_vcode_Token_2");
			pIpcam_st->onvif_st.vcode[i].w = 320;
			pIpcam_st->onvif_st.vcode[i].h = 240;
...

	{
		...
		pNetSt->net_dev = IPCAM_NET_DEVICE_WIRELESS;
		pNetSt->enable_wifi_auto_set_by_nvr = 1;
		pNetSt->ipv4[0] = 192;
		pNetSt->ipv4[1] = 168;
		pNetSt->ipv4[2] = 1;
		pNetSt->ipv4[3] = 60;
