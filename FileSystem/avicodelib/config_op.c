...
int co_get_key_value (char *key_line, char **mykey, char **myvalue)
{
    char* current;
    char* tail;
    char* value;
...
int co_etc_LocateKeyValue(FILE* fp, const char* pKey, 
                               ...
                               FILE* bak_fp, char* nextSection)
{
    ...
    while (1) {
        ...
        ret = co_get_key_value (szBuff, &current, &value);
        if (ret < 0)
            continue;
        else if (ret > 0) {
...
int co_etc_LocateKeyValue2(FILE* fp, const char* pKey, 
...
char* co_get_section_name (char *section_line)
{
    char* current;
    char* name;
...
int co_etc_LocateSection(FILE* fp, const char* pSection, FILE* bak_fp)
{
    ...
    while (1) {
        ...
        name = co_get_section_name (szBuff);
        if (!name)
            continue;
...
int co_GetValueFromEtcFile(const char* pEtcFile, const char* pSection,
                               const char* pKey, char* pValue, int iLen)
{
	...
	if (pSection)
	{
		if (co_etc_LocateSection (fp, pSection, NULL) != ETC_OK) 
		{
			fclose (fp);
			return ETC_SECTIONNOTFOUND;
		}

		if (co_etc_LocateKeyValue (fp, pKey, pSection != NULL, pValue, iLen, NULL, tempSection) != ETC_OK) 
		{
			fclose (fp);
			return ETC_KEYNOTFOUND;
		}
...
static void __mg_rewind (FILE *fp)
{
	rewind (fp);
}

static FILE* __mg_tmpfile (void)
{
	return fopen ("/tmp/mg-etc-tmp", "w+");  
}

static int __mg_close_tmpfile (FILE *tmp_fp)
{    
...
{    
	if (pSection && co_etc_LocateSection (etc_fp, pSection, tmp_fp) != ETC_OK)       
		return ETC_SECTIONNOTFOUND;    
	if (co_etc_LocateKeyValue (etc_fp, pKey, pSection != NULL, NULL, 0, tmp_fp, tempSection) != ETC_OK)        
		return ETC_KEYNOTFOUND;    
	return ETC_OK;
}

static int etc_CopyAndLocate2 (FILE* etc_fp, FILE* tmp_fp, const char* pKey, char* tempSection)
{    
	if (co_etc_LocateKeyValue2 (etc_fp, pKey,  NULL, 0, tmp_fp, tempSection) != ETC_OK)        
		return ETC_KEYNOTFOUND;    
	return ETC_OK;
}
...
int co_SetValueToEtcFile (const char* pEtcFile, const char* pSection, const char* pKey, char* pValue)
{   
	FILE* etc_fp;    
	FILE* tmp_fp;    
	int rc;    
...
int co_SetValueToEtcFile2 (const char* pEtcFile, const char* pKey, char* pValue)
{   
	FILE* etc_fp;    
	FILE* tmp_fp;    
	int rc;    
	...
	switch (etc_CopyAndLocate2 (etc_fp, tmp_fp,  pKey, tempSection)) 
	{    
		case ETC_SECTIONNOTFOUND:  			  
			fprintf (tmp_fp, "%s=%s\n", pKey, pValue);        
			break;        
