...
void executeCommand2(int value)
{
	char buf[200];
	int sockfd_client;
	...
	if ( ret > 0 && FD_ISSET(sockfd_client, &watch_set ) )
	{
		...
		recvbytes = recv(sockfd_client,buf,200,0);
...
int execute_command_thread_create2(int sockfd)
{
	printf("start to create execute_command thread\n");
	int ret = 0;
	ret = pthread_create(&g_executecommandpro, NULL,(void *)executeCommand2, sockfd);
	if ( ret < 0 )
	{
...
void RecvM2(int argc, char **argv)
{
	printf("into RecvM2\n");
	int listen_fd,i;
...
//bind , the ip and port information is aready in the sockaddr
	...
	while(1)
	{
		...
		if ( ret > 0 && FD_ISSET( listen_fd, &watch_set ) )
		{
			...
			execute_command_thread_create2(tmp_fd);
		}
	}
error_recv:
	if( listen_fd > 0 )
...
//connect
	...
	while(1)
	{
		...
		printf("wait command app return\n");
