...
static int main2(void)
{
  ...
  if((i=set_opt(fd,38400,8,0,'N',1))<0)
  {
...
static void create_socket_recv(int socket_fd)
{
	...
	ret = NM_client_socket_set2(&socket_fd,0,7,0);
	if( ret < 0 )
	{
	...
	ret = pthread_create(&netrecv_id,NULL,(void*)NM_net_recv_thread2,(void *)&socket_fd);
	if ( 0 != ret )
	{		
...
int main() 
{
	...
	open_audio_dev("/dev/dsp2");

	fp = fopen("audio.dat","wb");

	for( count = 0; count < 10 ;count++)
	{
		read_audio_buf(audio_buf,4096);

		fwrite(audio_buf,1,4096,fp);
	}
